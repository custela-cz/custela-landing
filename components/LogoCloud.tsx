export default function LogoCloud() {
  const logos = [
    { name: 'Google Ads', svg: GoogleAdsLogo },
    { name: 'Meta', svg: MetaLogo },
    { name: 'Sklik', svg: SklikLogo },
    { name: 'Google Analytics', svg: AnalyticsLogo },
  ]

  return (
    <section className="py-16 md:py-20 border-y border-black/[0.06] bg-[#FAFAFA]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <p className="text-center text-gray-muted text-xs font-medium uppercase tracking-wider mb-10">
          Integrujeme s platformami, které používáte
        </p>
        <div className="flex flex-wrap items-center justify-center gap-8 md:gap-16">
          {logos.map((logo) => (
            <div
              key={logo.name}
              className="flex items-center justify-center opacity-40 hover:opacity-80 transition-opacity"
            >
              <logo.svg />
            </div>
          ))}
        </div>
      </div>
    </section>
  )
}

function GoogleAdsLogo() {
  return (
    <div className="flex items-center gap-2 text-dark">
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
        <path d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .307 5.387.307 12s5.56 12 12.173 12c3.573 0 6.267-1.173 8.373-3.36 2.16-2.16 2.84-5.213 2.84-7.667 0-.76-.053-1.467-.173-2.053H12.48z" fill="currentColor"/>
      </svg>
      <span className="font-medium text-sm">Google Ads</span>
    </div>
  )
}

function MetaLogo() {
  return (
    <div className="flex items-center gap-2 text-dark">
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
        <path d="M12 2C6.477 2 2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.879V14.89h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.989C18.343 21.129 22 16.99 22 12c0-5.523-4.477-10-10-10z" fill="currentColor"/>
      </svg>
      <span className="font-medium text-sm">Meta Ads</span>
    </div>
  )
}

function SklikLogo() {
  return (
    <div className="flex items-center gap-2 text-dark">
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
        <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="2"/>
        <path d="M8 12h8M12 8v8" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
      </svg>
      <span className="font-medium text-sm">Sklik</span>
    </div>
  )
}

function AnalyticsLogo() {
  return (
    <div className="flex items-center gap-2 text-dark">
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none">
        <path d="M20 20H4V4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
        <path d="M4 16l6-6 4 4 6-6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
      </svg>
      <span className="font-medium text-sm">Analytics</span>
    </div>
  )
}
